...
private:
   ...
   Char_t  flag;               // User flags bit array (8 flags, one bit / flag)
                               ...
                               // Sixth bit reserved for rungeKutta track
                               // Seventh bit reserved for KalmanFilter track (not yet implemented)
                               // If flag for corresponding track =1, this track is accepted
   Bool_t isFakeInner;
   Bool_t isFakeOuter;
...
public:
   ...
   UChar_t getNShrHits(void) const              {return nShrHits;}  
   UChar_t getNTofHits(void) const              {return nTofHits;}
   
   Short_t getRpcClstInd(UChar_t n) const       {return n<nRpcClust ? rpcClstInd[n] : -1;}
   Short_t getShowerHitInd(UChar_t n) const     {return n<nShrHits  ? shrHitInd[n] : -1;}
   ...
   Float_t getShowerHitQuality(UChar_t n) const {return n<nShrHits  ? shrQuality[n] : -1.f;}
   ...
   Float_t getShowerHitDX(UChar_t n) const      {return n<nShrHits  ? shrDX[n] : -1000.f;}
   ...
   Float_t getShowerHitDY(UChar_t n) const      {return n<nShrHits  ? shrDY[n] : -1000.f;}
   ...
   Short_t getRungeKuttaIndShowerHit(UChar_t n)  const { return n < nShrHits  ? rkIndShower[n] : -1; }
   ...
   Short_t getKalmanFilterIndShowerHit(UChar_t n) const { return n < nShrHits  ? kfIndShower[n] : -1; }
