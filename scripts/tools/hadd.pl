...
if(!defined $ARGV[0] || !defined $ARGV[1] || $ARGV[0] =~ /help$/) {
  print "hadd.pl - wrapper for hadd for high input file count.\n";
  print "usage: hadd.pl  outputfile  inputfiles\n\n";
  print "Quick parallel merging of root files using multiple instances of hadd\n";
...
my $cmd = "ls -1 delme_hadd_temporary_files* 2>/dev/null";
my @o = qx($cmd);
if (scalar @o > 0) {
  print "Found files with name 'delme_hadd_temporary_files*' that may interfere with operation of this tool. Remove them.\n"; 
  exit -1;
  }
...
for(my $i = 0; $i<$max_parallel_tasks;$i++) {   
  ...
  if(!$p) {
    ...
    while($fp < $lastfile) {
      my $ofile = "delme_hadd_temporary_files_".$i."_$r";
      my $cmd = "hadd $ofile ";
...
$cmd = "hadd $output delme_hadd_temporary_files*";
system($cmd);
$cmd = "rm delme_hadd_temporary_files*";
system($cmd);
