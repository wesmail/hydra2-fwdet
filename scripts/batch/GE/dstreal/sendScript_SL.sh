...
submissiondir=${submmissionbase}/dstreal
 nFilesPerJob=1                                 # number of files to be analyzed by 1 job (default==1)
    ...
    outputdir=/lustre/nyx/hades/user/${user}/dst/apr12/gen8/$day     # outputdir for files AND logFiles
...
resources="--mem=2000 --time=0-10:00:00"                       # runtime < 10h, mem < 2GB

jobarrayFile="gen8_day_${day}_jobarray.dat"

filelist=day_${day}_nyx.list  # file list in local dir! not in submissiondir!!!
