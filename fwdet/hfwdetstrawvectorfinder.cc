...
Bool_t HFwDetStrawVectorFinder::init()
{
  ...
  fDiam = HFwDetStrawDigitizer::getRadius_S() * 2 / 10;
  ...
  */
  ...
  for (Int_t i = 0; i < fgkStat; ++i) {
    ...
    for (Int_t lay = 0; lay < fgkPlanes/2; ++lay) {
      ...
      //tr.print();
      Double_t layZ = tr.getTransVector().getZ() / 10; // layer position
      Double_t layDZ = TMath::Abs (vol->getPoint(0)->getZ()) / 10; // layer half-thickness
...
Bool_t HFwDetStrawVectorFinder::SelectHitId(HFwDetStrawCalSim *hit, Int_t idSel)
{
  ...
  for ( ; mit != hit->getDriftId().end(); ++mit) {
    if (mit->second == idSel) {
      ...
      Double_t err = hit->getDrift() * 10 - hit->getDriftId().begin()->first;
      Double_t coord = mit->first + err;
      coord = TMath::Max (0.0, coord);
      coord = TMath::Min (coord, fDiam/2*10);
      hit->setDrift(coord);
      return kTRUE;
    }
  }
