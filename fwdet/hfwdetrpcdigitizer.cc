...
HFwDetRpcDigitizer::HFwDetRpcDigitizer(void) {
    // default constructor
    initVariables();
}
...
void HFwDetRpcDigitizer::initVariables(void) {
...
Bool_t HFwDetRpcDigitizer::init(void) {
...
Int_t HFwDetRpcDigitizer::execute(void) {
	...
	if(ghit) {
	    ghit->getAddress(geantModule,geantCell);
            if(geantModule < 6) continue; // skip the other detectors of the FwDet
	    ...
	    trackNumber = ghit->getTrackNumber();
            ...
            if (module >= FWDET_RPC_MAX_MODULES || rpcNum >= FWDET_RPC_MAX_CELLS) continue;
            fLoc[0] = module;
	    fLoc[1] = rpcNum;
