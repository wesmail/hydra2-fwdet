...
class HGeantMergeSource : public HDataSource {
protected:
...
public:
    ...
    void     setRefId(Int_t runId, Int_t refId) { fRefIds[runId]=refId;  if(runId==fCurrentRunId) fCurrentRefId = refId; }
    void     setGlobalRefId (Int_t refId) { fGlobalRefId=refId;  if(fCurrentRefId==-1) fCurrentRefId = fGlobalRefId;  }
    ...
    void     Clear(void);
    void     Print() { HDataSource::Print();  if(fInput) {fInput->GetEntries(); fInput->ls();} }
